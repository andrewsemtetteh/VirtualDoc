...
export default function DoctorVerification({ darkMode }) {
  ...
  const openInfoRequestModal = (doctor) => {
    setSelectedDoctor(doctor);
  ...
  return (
    <div className={`p-6 rounded-lg ${
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                    <div className="flex space-x-2">
                      ...
                      <button
                        ...
                        onClick={() => openInfoRequestModal(doctor)}
                        disabled={actionInProgress}
                      >
                        {actionInProgress ? 'Processing...' : 'Request Info'}
                      </button>
                    </div>
                  </td>
                </tr>
      ...
      {showInfoRequestModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className={`p-6 rounded-lg ${darkMode ? 'bg-gray-800' : 'bg-white'} max-w-md w-full mx-4`}>
            <h3 className="text-lg font-medium mb-4">Request Additional Information</h3>
            <p className="text-sm text-gray-500 mb-4">
              Please specify what additional information you need from {selectedDoctor?.fullName}
            </p>
            <textarea
              ...
              rows="4"
              placeholder="Enter your information request..."
              disabled={actionInProgress}
            />
            <div className="flex justify-end space-x-2">
              <button
              ...
              <button
                onClick={() => handleAction(selectedDoctor._id, 'info_requested', infoRequestMessage)}
                className="px-4 py-2 bg-yellow-500 text-white rounded hover:bg-yellow-600 disabled:opacity-50 disabled:cursor-not-allowed"
                disabled={!infoRequestMessage.trim() || actionInProgress}
              >
                {actionInProgress ? 'Processing...' : 'Send Request'}
              </button>
            </div>
          </div>
        </div>
